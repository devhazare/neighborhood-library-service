...
def return_book(return_in: ReturnCreate, db: Session = Depends(get_db)):
    txn = borrow_service.return_book(db, return_in.borrow_id)
    data = borrow_repository.enrich_with_book_member(db, txn)
    return BorrowResponse(**data)

@router.get("/borrow/active", response_model=BorrowListResponse)
def list_active(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def list_overdue(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
